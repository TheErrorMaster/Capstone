...
def newWin4():
    ...
    win4.title("Four Player")
    win4.geometry("450x450")
    win4.config(bg="green")
    win4.resizable(False, False)
    roll_btn = Button(win4, text="roll", height=3, width=20, bg="blue", fg="white", command=randomGen).pack(pady=10)
    frame = Frame(win4)
    frame.pack()
    for i in range(0, 6):
        ans = Button(frame, textvariable=dice_rolled[i], height=1, width=2, font=("Helvetica", 20), command=lambda i=i : keep_dice(i)).pack(side=LEFT)
    result_lbls = Label(win4, textvariable=stringme, bg="green", fg="white", font=("Helvetica", 30)).pack()
